  ...
  >
    ...
    <div :class="infoClasses">
      ...
      <p
        ...
        class="mt-2 text-sm text-stone-500 line-clamp-2 leading-relaxed"
      >
        {{ project.description }}
      </p>
      ...
      <div v-if="project.styles && project.styles.length > 0 && variant !== 'compact'" class="mt-4 flex flex-wrap gap-1.5">
        <span
          ...
          :key="style.id"
          class="px-2.5 py-0.5 text-xs bg-stone-100 text-stone-500 rounded-full"
        >
          {{ style.name }}
        </span>
      </div>
...
const titleClasses = computed(() => {
  const base = 'font-semibold text-stone-800 group-hover:text-stone-700 transition-colors line-clamp-1'
